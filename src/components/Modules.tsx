...
export default function Modules() {
  return (
    <section id="modules" className="relative py-24">
      <div className="pointer-events-none absolute left-1/2 top-0 h-[400px] w-[700px] -translate-x-1/2 rounded-full bg-luna-500/15 blur-[140px]" />
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {modules.map((m, i) => (
            <Reveal
              key={m.title}
              delay={(i % 4) * 80}
              className="group glass relative overflow-hidden rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1.5 hover:border-luna-300/40"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-luna-500/20 text-luna-100 transition-colors group-hover:bg-gradient-to-br group-hover:from-luna-100 group-hover:to-luna-300 group-hover:text-luna-900">
                <m.icon className="h-6 w-6" strokeWidth={2.1} />
              </div>
              <h3 className="mt-5 text-base font-semibold text-cloud">
                {m.title}
              </h3>
